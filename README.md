...
| Telegram Command Bot with Slash Command Routing | Parses slash commands, strips the @botname suffix that groups append, and routes each command to its own branch. Handles the two things that actually break Telegram bots: one webhook per bot, and privacy mode hiding group messages. | Support | [Link to Template](Telegram/Telegram%20Command%20Bot%20with%20Slash%20Command%20Routing.json) |
...
| Validated Webhook Capture to Google Sheets | Captures form or webhook submissions, normalises and validates them before writing, and routes rejects to a separate sheet so bad payloads are visible instead of silently lost. | Ops | [Link to Template](Google_Drive_and_Google_Sheets/Validated%20Webhook%20Capture%20to%20Google%20Sheets.json) |
...
| Heartbeat Watchdog: Detect Workflows That Stopped Running Silently | Alerts on the absence of success rather than the presence of failure, catching what an Error Trigger structurally cannot: a workflow that never ran, a trigger left unpublished, or a poll that returns nothing and calls it success. Per-row staleness thresholds in Google Sheets, Slack alert. | IT Ops | [Link to Template](devops/heartbeat-watchdog.json) |
| Idempotency Guard: Stop Retries From Acting Twice | Builds a stable key per request, checks whether it was already handled, and only then acts - so a retry after a partial failure does not send the same invoice twice. Writes the key after the action, not before, which is the ordering most implementations get backwards. | IT Ops | [Link to Template](devops/idempotency-guard.json) |
| Error Handler: Alert With Workflow, Node, and Error Context | An error workflow that names the failing workflow, the node, and the message, so the alert tells you where to look instead of training you to ignore it. Set it as the Error Workflow on anything you care about. Slack enabled, Telegram included and disabled. | IT Ops | [Link to Template](devops/error-handler-alert.json) |
...
| Slack Approval Gate for Human-in-the-Loop Workflows | Pauses a workflow on a Slack message with Approve and Decline buttons and resumes down the matching branch once someone clicks, putting a person in front of anything irreversible. | Ops | [Link to Template](Slack/Slack%20Approval%20Gate%20for%20Human%20in%20the%20Loop%20Workflows.json) |
...
| AI Agent Starter with Memory and Calculator Tool | A minimal, current AI Agent wiring - chat trigger, model, window memory, and one tool - meant as the starting point to add your own tools to rather than a finished assistant. | Engineering | [Link to Template](OpenAI_and_LLMs/AI%20Agent%20Starter%20with%20Memory%20and%20Calculator%20Tool.json) |
| AI Classify and Route Incoming Messages | One model call sorts each incoming message into a fixed set of categories and a Switch node sends it to its own handler, with anything low-confidence falling through to a review sheet instead of being guessed at. | Support | [Link to Template](OpenAI_and_LLMs/AI%20Classify%20and%20Route%20Incoming%20Messages.json) |
